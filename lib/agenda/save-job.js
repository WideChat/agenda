...
/**
 * Given any object, replaces keys with assigned value `undefined`
 * with `null`
 *
 * We do this in order to prevent incorrect updates when the connection
 * to mongodb has `ignoreUndefined = true` configured (used in the `bson` lib)
 *
 * We found it makes more sense semantically; we're trying to "define" a value
 * for the field that represents something, we're not trying to "remove" the
 * field altogether. It's also important to notice that MongoDB would store the
 * value as `null` anyway if we send a field with `undefined` value, so there is
 * no impact in the logic.
 *
 * @param {*} obj
 * @returns {*}
 */
const replaceUndefinedForNull = obj => Object.entries(obj)
  .reduce(
    (acc, [key, val]) => ({...acc, [key]: typeof val === 'undefined' ? null : val}),
    {}
  );
...
module.exports = async function(job) {
  try {
    ...
    if (id) {
      // If `nextRunAt` is `undefined` and the MongoDB connection has
      // `ignoreUndefined = true`, the field would not be updated and
      // this would make the job run every time Agenda polled the DB
      // for jobs
      update.$set = replaceUndefinedForNull(update.$set);
